      ...
      >
        <div className="fixed inset-0 z-10 h-screen w-screen">
          <div className="flex min-h-full items-end justify-center text-center sm:items-center sm:p-0">
            ...
            >
              <div
                className={`flex shrink-0 px-4 ${currentQuestionIndex > 0 ? "justify-between" : "justify-end"}`}
              >
                  ...
                  <div className="flex flex-col justify-center items-center gap-8">
                                                  ...
                                                  prevProducts.map(
                                                    (product) => {
                                                      if (
                                                        ...
                                                        return {
                                                          ...product,
                                                          frequency: option.options?.[0]?.name,
                                                          frequencyLabel: option.options?.[0]?.label,
                                                          occurence: parseInt(
                                                            e.target.value,
                                                          ),
                                                        };
                                                  ...
                                                  prevProducts.map(
                                                    (product) => {
                                                      if (
                                                        ...
                                                        return {
                                                          ...product,
                                                          frequency: option.options?.[1]?.name,
                                                          frequencyLabel: option.options?.[1]?.label,
                                                          occurence: parseInt(
                                                            e.target.value,
                                                          ),
                                                        };
                                          ...
                                          (option, optionIndex) => (
                                            <div key={`product-${currentProductIndex}-subquestion-${subQuestionIndex + 1}-option-${optionIndex + 1}`} className="flex gap-2 mx-auto items-center py-4">
                                              ...
                                              <span className="p-lead text-v2-pink inline-block">
                                                {option.label}
                                                {
                                                    (selectedProducts[currentProductIndex]?.variants?.find(variant => variant.type === option.name)?.count ?? 0) > 1
                                                  ? "s" : ""
                                                }
                                              </span>
                                            </div>
                    ...
                    <div className="flex justify-center">
                      <button
                        ...
                        className={`pointer-events-auto inline-flex cta border-2 rounded-xl border-v2-blue text-v2-blue bg-v2-pink px-4 py-3 gap-4 ${!isNextButtonActive ? "opacity-50 cursor-not-allowed" : "hover:bg-v2-blue hover:text-v2-pink hover:border-v2-pink"}`}
                      >
                        <svg
                          width="24"
                          height="24"
    ...
    >
      <input
        ...
        onChange={(e) => {
          ...
          let variants: UserProductVariant[] = [];
          if (option.name === "smokedSalmon") {
            variants = DEFAULT_SMOKED_SALMON_VARIANT;
          }
          if (option.name === "sushi") {
            variants = DEFAULT_SUSHI_VARIANT;
          }
...
const CalculatorRadioInputBlock = ({
  ...
  return (
    <label key={`label-${questionId}`} className="cursor-pointer flex items-center gap-4">
      <input
        id={`question-${questionId}-option-${optionIndex + 1}`}
        key={`question-${questionId}-option-${optionIndex + 1}`}
        name="frequency"
          ...
          setSelectedProducts((prevProducts: UserProductConsumption[]) => {
            const updatedProducts = prevProducts.map((product) => {
              if (product.name === selectedProducts[currentProductIndex].name) {
                return { ...product, frequency: option.name, frequencyLabel: option.label };
              }
              return product;
            });
            return updatedProducts;
...
const SumUpBlock = ({
  ...
  selectedProducts: UserProductConsumption[];
  setIsLoadingScreenDisplayed: () => void;
}) => {
  const t = useTranslations("site.calculator");
  return (
    <div className="flex flex-col lg:grid grid-cols-2 justify-center lg:items-center px-4 gap-12 lg:gap-24 lg:pt-10">
      ...
      <div className="flex flex-col border-l-4 pl-4 border-v2-magenta gap-2 mx-auto lg:mx-0 lg:items-start">
        {selectedProducts.map((product, index) => {
          const filteredVariants = product.variants.filter(variant => variant.count > 0);
          return (
            <div key={`sumup-product-${index}`} className="flex flex-col justify-start pb-2 border-b-2 border-v2-magenta/20">
              <p className="p-lead text-v2-pink">
                {capitalizeFirstLetter(product.label!)}
              </p>
              <p className="p-caption px-2 text-v2-pink">
                {product.frequency === Frequency.OtherWeekly || product.frequency === Frequency.OtherYearly ? product.occurence : ""} {product.frequencyLabel}
              </p>
                      ...
                      filteredVariants.map((variant, variantIndex) => {
                        return (
                          <span key={`sumup-product-${index}-variant-${variantIndex}`} className="inline-block ml-1 p-caption text-v2-pink">
                            ...
                            { variantIndex < filteredVariants.length - 1 && variant.count >=  1 ? "," : ""} 
                          </span>
                        );
                      })
                    }
      ...
      <div className="flex flex-col justify-center items-center lg:items-start gap-2">
        <button
          type="button"
          onClick={() => setIsLoadingScreenDisplayed()}
