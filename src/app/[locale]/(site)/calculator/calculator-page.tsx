...
const CalculatorPage = () => {
  ...
  const [isQuestionModalOpened, setIsQuestionModalOpened] = React.useState(false);
  const [isLoadingScreenDisplayed, setIsLoadingScreenDisplayed] = React.useState(false);
  const questions = t.raw("modal.questions") as Question[];
  const [currentQuestionIndex, setCurrentQuestionIndex] = React.useState(0);
  const [selectedProducts, setSelectedProducts] = React.useState<UserProductConsumption[]>([]);
  ...
  const saveData = (newProduct: UserProductConsumption) => {
    if (selectedProducts.some(product => product.productName === newProduct.productName)) {
      setSelectedProducts((prevProducts) =>
        prevProducts.filter((product) => product.productName !== newProduct.productName)
      );
    } else {
  ...
  return (
    <>
      ...
      <Dialog open={isQuestionModalOpened} onClose={setIsQuestionModalOpened} className="relative z-50">
              ...
              <div className={`flex ${currentQuestionIndex > 0 ? "justify-between" : "justify-end"}`}>
                {
                  currentQuestionIndex > 0 && (
                    <ArrowLeft onClick={() => setCurrentQuestionIndex(currentQuestionIndex - 1)} className="text-v2-pink text-end cta" /> 
                  )
                }
                <CloseButton onClick={() => {
              ...
              </div>
              {
                isLoadingScreenDisplayed ? (
                    <div className="flex flex-col justify-center items-center gap-8">
                      ...
                      <p className="p-caption italic text-v2-pink">{t("loading.info")}</p>
                    </div>
                  ) : (
                    <div className="flex flex-col justify-center items-center gap-8">
                      <div className="flex flex-col justify-center gap-8">
                        {
                          questions.slice(currentQuestionIndex, currentQuestionIndex + 1).map((question, index) => (
                            <div key={index} className="flex flex-col h-full items-center px-6 gap-10">
                              <DialogTitle className="h3 lg:h2 text-center text-pretty text-v2-pink">{question.title}</DialogTitle>
                              <div className="flex flex-col w-full gap-4">
                                {question.options.map((option, optionIndex) => {
                                  if (option.name === "otherType") {
                                    return (
                                        <input
                                          id={`question-${optionIndex+1}`}
                                          key={`question-${optionIndex+1}`}
                                          name={option.name}
                                          onChange={() => {
                                          ...
                                          <div className="inline-flex justify-center items-center gap-1">
                                            <input
                                              ...
                                              className="
                                                ...
                                                focus:text-v2-pink"
                                            />
                                            <span className="p-lead text-v2-pink inline-block">{option.options ? option.options[0].label : null}</span>
                                            <span className="text-2xl text-v2-pink inline-block">/</span>
                                            <input
                                              id={`offers-${optionIndex}`}
                                              name={option.name}
                                              type="text"
                                              ...
                                              className="
                                                ...
                                                focus:text-v2-pink"
                                            />
                                            <span className="p-lead text-v2-pink inline-block">{option.options ? option.options[1].label : null}</span>
                                          </div>
                                  ...
                                  return (
                                    <label key={`label-${optionIndex+1}`} className="flex items-center gap-4">
                                    <input
                                      id={`question-${optionIndex+1}`}
                                      key={`question-${optionIndex+1}`}
                                      name={question.type === "radio" ? question.title : option.name}
                                      onChange={() => {
                                        saveData({ productName: option.name });
                                      }}
                                      checked={selectedProducts.some(product => product.productName === option.name)}
                                      type={question.type}
                                      aria-describedby="offers-description"
                                      className="
                                        appearance-none
                                    ...
                                    />
                                    <span className="p-lead text-v2-pink">{capitalizeFirstLetter(option.label)}</span>
                                  </label>
                                  );
                                })}
                              </div>
                      ...
                      <div className="flex justify-center">
                        <button
                          ...
                          className="inline-flex cta border-2 rounded-xl pointer-events-auto hover:scale-105 border-v2-blue text-v2-blue bg-v2-pink px-4 py-3 gap-4"
                        >
                          <svg
                            width="24"
                            height="24"
